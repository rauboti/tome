...
- [X] T122 [US1] Sealed **`CharacterData`** hierarchy — finish the maintainer's started files in `api/src/main/kotlin/no/rauboti/tome/characters/data/`: add the missing `package no.rauboti.tome.characters.data`; make `DSCharacterData.kt` declare its **own** `DarkSoulsCharacterData` (currently a duplicate of the DnD35 class — a redeclaration error); model **`DnD35CharacterData`** with base-input constructor properties + **derived computed `val`s** (the 3C content), and **typed row data classes** for the tables (skills/attacks/feats/gear/spell-slots/spells). Add `@TypeAlias("dnd35")`/`@TypeAlias("darksouls")` (storage discriminator value) and Jackson `@JsonTypeInfo(use = NAME, property = "ruleSetId")` + `@JsonSubTypes` (wire polymorphism). `DarkSoulsCharacterData` stays a **minimal stub variant** so `when`s compile (US5 fleshes it, T072–T075). Depends on T118. **Impl notes (2026-07-24, TDD, branch `001-campaign-management-t122-typed-charactersheet`):** _tests-first (red → green)_ — wrote 3 test classes against the intended API, confirmed red (compile: all "unresolved reference" to the not-yet-existent model, nothing else), then implemented to green. **Which T119–T121 tests landed here** (the model-in-isolation slice): the whole of **T120** = `DnD35CharacterDataTest` (10 cases, computed-derived **parity** ported 1:1 from `DnD35RuleSetTest`'s compute assertions — ability mods, saves, initiative, AC/touch/flat-footed, grapple, skill total, attack bonus, gear `totalWeight`, spell-slot bonus/total incl. level-0 zeroing); plus the **model-level slice of T119** = `CharacterDataSerdeTest` (5, pure Jackson wire polymorphism — discriminator on the wire, deserialize-by-`ruleSetId`, unknown id rejected, round-trip) and `CharacterDataPersistenceTest` (3, real-Mongo round-trip via a test-local `SheetHolder` doc — `_class`=`@TypeAlias`, derived not stored). The **HTTP/service parts of T119** (MockMvc create→edit→reload, 409, controller) stay for **T124**; **T121** is web. _model — base/enriched split (maintainer design 2026-07-24, evolved past the single-type grouping)_ — **two** sealed hierarchies: **`CharacterBaseData`** (the stored/parsed/derived-from side; `@TypeAlias` storage discriminator + `@JsonTypeInfo` request discriminator on `ruleSetId`) with `DnD35CharacterBaseData` = identity top-level + grouped **base-input** value objects (`DnD35AbilityScores`, `DnD35HitPoints`, `DnD35DefenseInputs`, `DnD35SaveInputs`, `DnD35SpellcastingInputs` = casterClass/casterLevel/spellKeyAbility/spellSlots/spells) + `...Input` row types; and **`CharacterData`** (the served side; `@JsonTypeInfo` response discriminator) with `DnD35CharacterData(base)` = the **enriched** view exposing groups **with derived** the way the sheet reads — `abilities` (scores+mods), `defense` (+AC/touch/flat-footed), `saves` (+fortitude/reflex/will), `spellcasting` (+saveDcBase + per-slot bonus/total), and enriched rows carrying per-row totals (`skills[].total`, `attacks[].attackBonus`). `CharacterBaseData.enrich()` is the exhaustive `when` mapping base→enriched. **This cleanly solves the cross-object-derived problem** (the enriched view is built on read with access to every group — no reconstruction/injection tricks) **and per-row derived** (enriched rows carry totals). The base type has **zero** derived properties, so nothing derived can leak to storage — a stronger, trivial guarantee. Jackson annotations `com.fasterxml.jackson.annotation.*` (Jackson 3 reuses the 2.x pkg; `EXISTING_PROPERTY` on `ruleSetId`; verified); `class`→`characterClass` (Kotlin keyword); `DSCharacterData.kt` → `DarkSoulsCharacterBaseData` + `DarkSoulsCharacterData` stubs. **6 source files.** **Verified:** `DnD35CharacterDataTest` + `CharacterDataSerdeTest` = **15/15 green**, Spotless clean, test-compile green. ⚠ **`CharacterDataPersistenceTest` (3) NOT yet executed — Docker is down in this session** ("Could not find a valid Docker environment for Testcontainers"); the model is proven by the spike's identical mechanism + the pure tests, but **run these once Docker is up** to close the storage assertions.
- [X] T123 Reshape the **`RuleSet`** strategy for the typed world in `api/src/main/kotlin/no/rauboti/tome/rulesets/` — drop `computeDerived(SheetData)` and `definition(): SheetDefinition` (compute is now the sheet's own properties); keep `id()`/`name()` and `validate(...)` **retyped to read the typed sheet** (soft warnings, FR-005, unchanged behavior). Decide the fate of **`GET /api/rule-sets/{id}`**: the web no longer renders from a definition, so this shrinks to serving **id + name** for the create-character picker (drop the `SheetDefinition` body); `RuleSetRegistry` unchanged. Retire `SheetData`/`SheetDefinition`/`SheetField`/`SheetChange`/`OptionsFrom`/`FieldType` from `RuleSet.kt` (what survives moves with the type). Depends on T122. **Done as part of the combined T123+T124 slice (2026-07-24, branch `001-campaign-management-t123-t124-typed-slice`) — reshaping `RuleSet` breaks its only consumers (the character slice), so the two had to land together to compile (Gaute's call).** Impl: `RuleSet` interface → `id()`/`name()` + `validate(sheet: CharacterBaseData): List<RuleWarning>` (dropped `computeDerived`/`definition`; removed `SheetChange`). `DnD35RuleSet` now **no-arg** (no `objectMapper`/`definition.json`), `validate` ported to read the typed `DnD35CharacterBaseData` (ability<1, level<1, skill-ranks-over-max — same warnings/codes/fields as before). `RuleSetController` `GET /{id}` → **`RuleSetSummary`** (id+name), not `SheetDefinition`. **Left as dead code for T125** (kept compiling, unused by the live path): `SheetData`/`SheetDefinition`/`SheetField`/`FieldType`/`OptionsFrom`/`FieldOption`/`SheetSection` in `RuleSet.kt`, `SheetCompute`, `FormulaEvaluator`, `definition.json` (+ their still-green tests `SheetComputeTest`/`FormulaEvaluatorTest`). Tests: `DnD35RuleSetTest` → 6 typed-`validate` cases (pure; the old definition-shape/`computeDerived` cases retired — parity now in T122's `DnD35CharacterDataTest`); `RuleSetControllerTest` `/{id}` → summary.
- [ ] T124 [US1] Retype the character slice on the **base/enriched split** (T122) — `Character.data: CharacterBaseData` (was `SheetData`) in `Character.kt` (stores base inputs only); `CharacterRepository` largely unchanged (spike proved the round-trip); **rewrite `CharacterService`**: **delete `stripDerived`** (the base type has no derived — nothing to strip) and **delete `CharacterDataResolver`** (replaced by `CharacterBaseData.enrich()` — the read path enriches base→`CharacterData` for the response), `validate` reads the base (or enriched) typed sheet; `CharacterController` returns the **enriched** `CharacterData` (HP from `enriched.hitPoints`, not `data["hpCurrent"]`). Delete `CharacterDataResolver.kt` + `CharacterDataResolverTest.kt`. **Lock the request shape (decided 2026-07-24; deferred from T118):** POST/PUT request `data` binds to the polymorphic **`CharacterBaseData`** (base inputs; no derived on that type at all), replacing T118's loosely-typed `object`. **openapi: two schemas** — a **base input schema** (request `data`, the grouped inputs) and the **enriched response schema** (grouped, derived nested in groups + rows), replacing the single flat `DnD35Sheet`; `class`→`characterClass`; the web codegens both (T126). Enforce on **both ends**: server binds `data` polymorphically (Jackson `@JsonTypeInfo` on `ruleSetId`) and **rejects an unknown/mismatched rule set → 400**; reconcile the create request's discriminator with the top-level `ruleSetId` (require `data.ruleSetId` + 400 on disagreement, vs. inject the resource's — pick the simpler, document it). This is what makes "enforce the DnD35 vs. DarkSouls shape on both client and server" real. Depends on T122/T123; makes T119 pass (incl. unknown/mismatched-`ruleSetId` → 400). **Done in the combined T123+T124 slice (2026-07-24).** Impl: `Character.data: CharacterBaseData` (stored base). Deleted `CharacterDataResolver` + `CharacterDataResolverTest` (replaced by `CharacterBaseData.enrich()`). `CharacterService` typed — no `stripDerived` (base has no derived), `validate(base)`, **FR-002 guard** (update with a differing `data.ruleSetId` → 400), create resolves the rule set from `data.ruleSetId` (unsupported → 400). `CharacterController`: **`CreateCharacterRequest {name, data: CharacterBaseData}`** (dropped the redundant top-level `ruleSetId`; the sheet's type carries it) + `UpdateCharacterRequest {name?, data: CharacterBaseData, version}`; `data` binds **polymorphically** on `ruleSetId`, so an unknown rule set fails to bind → **400** (enforced server-side). Response is the **enriched `CharacterData`** (`character.data.enrich()`); **dropped promoted `hpCurrent`/`hpMax`** (HP lives in `data.hitPoints`). **T119 tests rewritten + green:** `CharacterIntegrationTest` (6 — typed base request, enriched grouped derived in response `data.abilities.strMod`/`data.saves.fortitude`, raw stored doc has grouped base + no derived, soft warning, **unknown ruleSetId→400**, stale `@Version`→409) and `CharacterContractTest` (11 — typed bodies, +unknown-ruleSet 400). Behaviour changes noted: no name auto-seed into `data` (top-level `Character.name` is authoritative); the skill/ability `validate` now always applies (typed fields always present, sensible defaults). **Verified: `./mvnw clean test` → 74 green** (also closed T122's `CharacterDataPersistenceTest` now Docker is up), Spotless clean. **openapi reshaped (2026-07-24, same slice):** replaced the single flat `DnD35Sheet` with the **two-schema split** — `SheetInput` (oneOf `DnD35SheetInput`/`DarkSoulsSheetInput`, the request/stored base, grouped `abilities`/`hitPoints`/`defense`/`saves`/`spellcasting` + `…RowInput` rows, no derived) and `Sheet` (oneOf `DnD35Sheet`/`DarkSoulsSheet`, the enriched response, grouped with `readOnly` derived + enriched rows). Create request → `{name, data: SheetInput}` (**dropped top-level `ruleSetId`**); PUT/NPC request `data` → `SheetInput`; `Character.data`/`Npc.data` → enriched `Sheet`; `class`→`characterClass`; `GET /rule-sets/{id}` → `RuleSetSummary` (from T118). Validated: YAML parses, **zero dangling `$ref`s**. This is the codegen source for **T126**. **`SheetData`/`SheetChange`/`definition.json` + the formula engine are now dead — T125 deletes them.**
