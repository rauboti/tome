...
- [ ] T112 Spellcasting **spell catalog**, canonical (baked SRD) — the deferred half of T111 (Gaute: "baked SRD catalog"). A `spells` table where the spell name is **pick-from** the 3.5 **SRD** spell list (+ per-row level/prepared/notes). **Must be spec-planned first** (its own re-plan, like Phase 3C): _(a) sourcing_ — the ~600 SRD spells MUST be **sourced from an authoritative OGL dataset** (fetch/parse), NOT hand-authored (hand-typing risks omissions/wrong entries); _(b) data shape_ — a **companion resource** (e.g. `rulesets/dnd35/spells.json`) the rule set loads, NOT inline in `definition.json`; _(c) per-class structure_ — a spell's level varies by class; decide flat catalog + user-entered per-row level vs. per-class lists; _(d) filtering_ — a class-filtered spell select needs a **new widget capability** (options depending on the `casterClass` value; the current `select` is static) or accept one flat ~600-entry select. Depends on T105/T110/T111.

**Checkpoint**: the 3.5 sheet is as rich as v1 wants (spell *catalog* T112 pending); US1 still green; US2 can begin.
